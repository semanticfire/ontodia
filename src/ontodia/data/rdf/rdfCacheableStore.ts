...
export function isLiteral(el: Term): el is Literal {
    return el.termType === 'Literal';
}

export function isNamedNode(el: Term): el is NamedNode {
...
export class RDFCacheableStore {
    ...
    matchAll(statements: MatchStatement[]): Dataset {
        const slowQueries: MatchStatement[] = [];
        const responses: Dataset[] = [];
    ...
    private mergeDatasets(datasets: Dataset[]): Dataset {
